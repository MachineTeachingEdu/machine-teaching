...
def get_problem(problem_id):
    ...
    solution_python = Solution.objects.filter(problem=problem, ignore=False, language=Solution.PYTHON).order_by('?')[0]
    
    dictSolutions = {}
    solutions = Solution.objects.filter(problem=problem, ignore=False)  #Pegando soluções de todas as linguagens
    for solution in solutions:
        dictSolutions[solution.language] = {"solution": solution.content, "text": solution.tip, "return_type": solution.return_type, "func": solution.header}
